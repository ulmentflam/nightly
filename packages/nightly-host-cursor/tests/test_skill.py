"""Sanity checks on the SKILL.md content shipped to Cursor."""
...
def test_skill_md_mentions_cursor_specifics() -> None:
    # Cursor-flavored dispatch (Background Agents) + lifecycle notes
    assert "Background Agent" in SKILL_MD
    assert ".cursor/commands/nightly.md" in SKILL_MD
    assert "Cursor" in SKILL_MD


def test_skill_md_acknowledges_no_local_os_sandbox() -> None:
    body = SKILL_MD.lower()
    assert "no os" in body or "no os sandbox" in body or "no os-level" in body
