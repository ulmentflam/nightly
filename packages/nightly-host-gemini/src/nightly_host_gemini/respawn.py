"""Respawn launcher for Gemini CLI — not yet implemented (RFC 010 v2).
...
operator who sets `supervisor.host: gemini` deserves to learn on the
first respawn attempt, not to discover months later that their
overnight runs were never being resumed.

What blocks v2 here:
Gemini CLI's `AfterAgent` hook writes the marker, so the trigger works. The launcher needs `gemini`'s interactive-resume invocation, which differs from the `--prompt` headless form Nightly already uses.
"""
